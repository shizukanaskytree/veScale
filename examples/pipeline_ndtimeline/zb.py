...
#     def __init__(self, hidden):
#         super().__init__()
#         self.mlps = nn.ModuleList([MLP(hidden) for _ in range(8)])  # Changed to ModuleList
...
#         sys.exit(f"Not enough GPUs available for the required world size: {world_size}")
...
#     # Set the device for NCCL process group for collectives
#     if backend == "nccl":
#         torch.cuda.set_device(rank)
...
# def test_zerobubble_engine(backend):
#     """
#     Tests zero-bubble pipeline schedule with profiling.
#     """
#     world_size = int(os.environ["WORLD_SIZE"])
...
#     # Auto select device and backend
...
#         # Synchronize before starting the training loop
#         torch.distributed.barrier()
...
#     test_zerobubble_engine(backend)
